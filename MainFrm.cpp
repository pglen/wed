...
/* =====[ mainfrm.cpp ]==========================================
   ...
   Description:     The wed project, implementation of the mainfrm.cpp
...
BEGIN_MESSAGE_MAP(CMainFrame, CMDIFrameWnd)
    ...
    ON_COMMAND(ID_FILE_OPEN, OnOpensource)
    ON_WM_TIMER()
...

{
    ...
    if(!SetTimer(2, 3000, NULL))
        {
        AfxMessageBox("Cannot create timer, autohide will not be available");
        }
...

{
    if( pMsg->message == WM_KEYDOWN)
        {
        //P2N("CMainFrame::PreTranslateMessage %d\r\n", pMsg->message);
        if(spp.splashed)
            spp.Hide();
        }
